...
// custom control para leyenda
  ...
  var legendHtml= '';
   ...
   legendHtml += '<div class="col-md-10" style="padding-left:5px"><h4 style="margin-top:0px; margin-bottom:5px;">Legend</h></div>'+
          ...
          '</div>'+
          '<div id="legend1">'+
          '<div class="col-md-12 icon"> <img width="20px" height="15px" style="margin-left:0px; margin-right:5px; margin-bottom:2.5px" src="img/castle.png"/>'+
          '<span style="font-size:15px">Castillo</span></div>'+
  ...
  var fuente=L.geoJson(fuentes,{pointToLayer:function(geoJsonPoint, latlng) {
    var popup = L.popup()
    .setLatLng(latlng)
    .setContent('<p>Holaaaaaa!<br />Prueba de info pop up</p>');
    return L.marker(latlng,{icon:myIconfuente}).bindPopup(popup);
  ...
  var castillo=L.geoJson(Castillo,{pointToLayer:function(geoJsonPoint, latlng) {
    var popup = L.popup()
    .setLatLng(latlng)
    .setContent('<p>Hello world!<br />This is a nice popup.</p>');
    return L.marker(latlng,{icon:myIconcas}).bindPopup(popup);
  //buena forma con each poner 
}
}).addTo(map);
  var overlay ={
